...
void check_stall() {
	if(em.inst.a != NOP && em.memLoad && (em.rt == de.rs || em.rt == de.rt)) {
    // Stall a cycle
		fd.inst = de.inst;
		fd.valP = de.valP;
		de.inst.a = NOP;
...
void forward() {
  bool_t emHazard = (em.inst.a != NOP) && (!em.memLoad) && (
  ...
  // em harzard
  if(emHazard) {
    if ((em.writeRT && em.rt == de.rs) ||
    (em.writeRD && em.rd == de.rs)) {
      // forward to RS
      de.valS = em.valE;
    } else if ((em.writeRT && em.rt == de.rt) ||
    (em.writeRD && em.rd == de.rt)) {
      // forward to RT
      de.valT = em.valE;
    }
  } else if(mwHazard) {
    if ((mw.writeRT && mw.rt == de.rs) ||
    (mw.writeRD && mw.rd == de.rs)) {
      // forward to RS
      de.valS = mw.memLoad?mw.valM:mw.valE;
    } else if ((mw.writeRT && mw.rt == de.rt) ||
    (mw.writeRD && mw.rd == de.rt)) {
      // forward to RT
      de.valT = mw.memLoad?mw.valM:mw.valE;
    }
  }
  return;
}
